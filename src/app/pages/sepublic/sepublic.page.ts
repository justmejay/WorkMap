...
@Component({
  selector: 'app-sepublic',
  templateUrl: './sepublic.page.html',
  styleUrls: ['./sepublic.page.scss'],
})
export class SepublicPage implements OnInit {
...
console.log(this.checkeditems);
    if (this.checkeditems == "" ){
      ...
      this.app.getjoblistall().pipe(first()).subscribe(res =>{
        this.listdata = res;
        ...
        if (this.filter == 1){
          ...
          this.finaldata.sort((n1,n2) => n2.timesort - n1.timesort);
  
        }else if (this.filter == 3){
  
          this.finaldata.sort((n1,n2) => n2.jsalary - n1.jsalary);
  
  
  
  
        }else if (this.filter == 4){
          this.finaldata.sort((n1,n2) => n1.jsalary - n2.jsalary);
        }
  
        for (let x = 0; x<this.finaldata.length; x++){
  
          this.profile.getcoords().pipe(first()).subscribe(res=>{
              this.fpdata = res;
    
              this.maps.distancecompute(this.finaldata[x].lat, this.finaldata[x].lng, this.fpdata.clat, this.fpdata.clng).pipe(first()).subscribe(res =>{
    
                this.finaldata[x].distance = res.rows[0].elements[0].distance.text;
                this.finaldata[x].distancesort = res.rows[0].elements[0].distance.value;
